...
export default function AddPriorityModal({ isOpen, onClose, onSubmit }: AddPriorityModalProps) {
  ...
  const loadDropdownData = async () => {
    ...
    try {
      ...
      if ((subjectsResponse.results?.length === 0 || subjects.length === 0) && 
          (timeSlotsResponse.results?.length === 0 || timeSlots.length === 0)) {
        alert('No subjects or time slots found. Please add subjects and time slots first.');
      } else if (subjectsResponse.results?.length === 0 || subjects.length === 0) {
        alert('No subjects found. Please add subjects first before creating priorities.');
      } else if (timeSlotsResponse.results?.length === 0 || timeSlots.length === 0) {
        alert('No time slots found. Please add time slots first before creating priorities.');
      }
    } catch (error) {
